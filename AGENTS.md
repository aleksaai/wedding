...
- The private guest workspace lives in the same application at `/admin`. It uses Supabase email magic-link authentication and supports invitation creation/editing, category/language/status filters, card/sent/opened tracking, direct-link copy, and response review. Approved emails are `aleksa.spalevic2000@gmail.com` and `szildebora@gmail.com`.
